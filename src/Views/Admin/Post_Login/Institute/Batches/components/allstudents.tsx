...
const Allstudents = () => {
  ...
  // Assign Batch modal data - 3 dummy batches
  const assignBatches = [
    { id: 1, name: 'English Batch 2025', code: 'ENGB25' },
    { id: 2, name: 'Mathematics Batch 2025', code: 'MATHB25' },
    { id: 3, name: 'Science Batch 2025', code: 'SCIB25' },
  ];
  ...
  const handleAssignBatchModalCancel = () => {
    setAssignBatchModal(false);
    setSelectedBatches([]);
  };

  const handleBatchSelection = (batchId, checked) => {
    if (checked) {
  ...
  const handleAssignBatches = () => {
    if (selectedBatches.length === 0) {
      message.warning('Please select at least one batch to assign');
      return;
    }
    
    const selectedBatchNames = assignBatches
      .filter(batch => selectedBatches.includes(batch.id))
      .map(batch => batch.name)
      .join(', ');
    
    message.success(`Successfully assigned to: ${selectedBatchNames}`);
    setAssignBatchModal(false);
    setSelectedBatches([]);
  };
  ...
  return (
    <div className="p-4 md:p-6 h-min">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              ...
              style={{ borderRadius: '6px' }}
            >
              Batch not assigned({getPendingCount()})
            </Button>
            <Button
      ...
      <Modal
        title="Assign Batch"
        open={assignBatchModal}
        onCancel={handleAssignBatchModalCancel}
        footer={null}
        width={500}
        centered
        className="font-local2"
      >
        <div className="space-y-4">
          <div className="text-center mb-4">
            <Text className="text-gray-700 text-base font-local2">
              Select the batch(es) you want to assign to this student:
            </Text>
          </div>
          
          <div className="space-y-3">
            {assignBatches.map((batch) => (
              <div key={batch.id} className="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg">
                <Checkbox
                ...
                />
                <div className="flex-1">
                  <Text className="text-gray-800 font-medium font-local2 block">
                    {batch.name}
                  </Text>
                  <Text className="text-gray-600 text-sm font-local2">
                    Code: {batch.code}
          ...
          <div className="flex gap-3 pt-4">
            <Button
              onClick={handleAssignBatchModalCancel}
              className="flex-1 font-local2"
              size="large"
              style={{ height: '40px', borderRadius: '8px' }}
            >
            ...
            <Button
              ...
              type="primary"
              className="flex-1 font-local2 bg-blue-500 hover:bg-blue-600 border-0"
              size="large"
              style={{ height: '40px', borderRadius: '8px' }}
            >
              Assign
